...
def test_create_v3_service_application(respx_mock, metadata_v3):
    """Check V3 client creation when metadata is fetched over httpx sync."""

    headers = httpx.Headers({'Content-Type': 'application/xml'})
    respx_mock.get(f"{SERVICE_URL}/$metadata").mock(
        return_value=Response(status_code=200, content=metadata_v3, headers=headers)
    )

    client = pyodata.Client(SERVICE_URL, httpx, odata_version=Client.ODATA_VERSION_3)

    assert isinstance(client, pyodata.v3.service.Service)
    assert client.schema.entity_type('Document').name == 'Document'


def test_v3_get_entity_uses_verbose_json_headers(respx_mock, metadata_v3):
    """Check a representative V3 request path and header policy for httpx sync."""
