...
- Preserve alias handling, V3 namespace acceptance, the current stream-related metadata exposure (`HasStream` and `Edm.Stream`), open-type metadata exposure (`OpenType`), the current V3-only primitive registration slice used by the tests (including the current opaque spatial-type handling), and the current `FunctionImport` metadata shape used by the V3 service layer, including bindability flags, binding-parameter identification, entity-set-path data, and container-name metadata used for bound actions.
